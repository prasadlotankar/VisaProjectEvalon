...
class QualityCheck:
    ...
    def perform_qc(self, spark):
        try:
            ...
            decrypted_data = spark.read.json(self['DEFAULT']['DirectoryPath'])
            decrypted_data.write.mode("overwrite").json(self['Paths']['qc'])
            ...
            json_files = [i for i in os.listdir(self['Paths']['qc']) if i.endswith('.json')]

            # Iterate through each .json file, calculate MD5, and save QC data
            for file_name in json_files:
                file_path = os.path.join(self['Paths']['qc'], file_name)
